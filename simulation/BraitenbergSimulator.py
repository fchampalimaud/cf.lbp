...
from PySide6.QtGui import QPainter, QPen, QBrush, QColor, QFont, QDrag
...
class SimulatorApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Braitenberg Simulator")
        self.resize(950, 800)
    ...
    def _build_world_group(self, panel_vl=None):
        ...
        self._obj_btns = {}
        for letter, name, color, bg in OBJECT_COLORS:
            btn = QPushButton(letter)
            btn.setFixedSize(26, 26); btn.setCheckable(True)
            btn.setStyleSheet(
                f"background:{bg};border:2px solid {C['border']};border-radius:3px;font-weight:bold;")
            btn.clicked.connect(lambda checked, c=color, l=letter: self._set_object_mode(c, l))
            ol.addWidget(btn)
    ...
    def _set_object_mode(self, color, letter):
        self._editor.set_object_mode(color, letter) if hasattr(self, '_editor') else None
        self._clear_mode_buttons()
