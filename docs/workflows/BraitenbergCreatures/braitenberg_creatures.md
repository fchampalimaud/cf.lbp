# Braitenberg vehicles

Braitenberg’s Vehicles (1984) demonstrate how complex, lifelike behaviors can arise from minimal sensorimotor architectures. 

<div style="display: flex; justify-content: center; gap: 10px;">
    <img src="Braitenbergs1.svg" alt="Braitenberg1" style="transform: rotate(10deg); height: 300px;">
</div>
<p style="text-align: center; font-size: 18px; font-weight: bold; margin-top: 20px;">Figure1: caption for Braitenberg1</p>


**Vehicle 1** is the simplest form: sensors are directly linked to motors so that the vehicle’s speed increases with light intensity. This basic arrangement produces straightforward phototaxis—movement guided purely by brightness—showing how even a single feedback loop can generate purposeful motion without any form of perception or cognition.

<div style="display: flex; justify-content: center; gap: 50px;">
    <img src="Braitenbergs2a.svg" alt="Braitenberg2a" style="transform: rotate(-10deg); height: 300px;margin-right: 100px;">
    <img src="Braitenbergs2b.svg" alt="Braitenberg2b" style="transform: rotate(10deg); height: 300px;">
</div>
<p style="text-align: center; font-size: 18px; font-weight: bold; margin-top: 20px;">Figure2: caption for Braitenbergs2a and Braitenbergs2b</p>


**Vehicles 2a** and **2b** expand on this idea by adding two sensors and two motors, connected either on the same side or crossed to the opposite side. In Vehicle 2a, uncrossed connections cause the vehicle to turn away from light, mimicking “fear.” In Vehicle 2b, crossed wiring produces the opposite reaction: the vehicle turns toward light, an “aggressive” or seeking behavior. These configurations reveal how directional approach and avoidance can emerge simply from geometric differences in wiring.

<div style="display: flex; justify-content: center; gap: 30px;">
    <img src="Braitenbergs3a.svg" alt="Braitenberg3a" style="transform: rotate(-10deg); height: 300px;margin-right: 100px;">
    <img src="Braitenbergs3b.svg" alt="Braitenberg3b" style="transform: rotate(10deg); height: 300px;">
</div>
<p style="text-align: center; font-size: 18px; font-weight: bold; margin-top: 20px;">Figure3: caption for Braitenbergs3a and Braitenbergs3b</p>

With **Vehicles 3a** and **3b**, Braitenberg introduces inhibition, adding another layer of complexity. Here, the sensors decrease rather than increase motor activity. In 3a, same-side inhibitory wiring causes the vehicle to slow and stop near a light source—interpreted as “love.” In 3b, crossed inhibitory connections yield continual adjustments and wandering paths, behaviorally resembling “exploration.” Across this series, each new design demonstrates how variations in sensor–motor coupling can produce behaviors that seem intentional or emotional, yet arise purely from feedback dynamics.


<div style="display: flex; justify-content: center; gap: 30px;">
    <img src="Braitenbergs3c.svg" alt="Braitenberg3a" style="transform: rotate(-10deg); height: 180px;">
</div>
<p style="text-align: center; font-size: 18px; font-weight: bold; margin-top: 20px;">Figure3: caption for Braitenbergs3c</p>


**Vehicle 3c** merges the principles of **Vehicles 2** and **3**, incorporating both approach-avoidance dynamics and inhibitory feedback into a more integrated system. This configuration allows for multiple feedback loops to be triggered across different sensory modalities, such as light or proximity. The result is a vehicle that exhibits behaviors summing at the motor level, where various combinations of sensory input lead to distinct motor outputs. Depending on the sensory stimuli, the vehicle may alternate between attraction and avoidance, or exhibit more nuanced movements, responding differently to varying intensities of light, distance, or other environmental factors. This setup demonstrates how complex, adaptive behaviors can emerge from the interaction of simple feedback systems, leading to a wider range of responses to stimuli.






<!--
![Schematics](./Braitenbergs2a.svg){ height=2% }

![Schematics](./Braitenbergs1.svg){ height=2% }
-->



## Workflow
:::workflow
![Example](~/workflows/HarpExamples/BehaviorBoard/AnalogInput/AnalogInput.bonsai)
:::
